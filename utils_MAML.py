...
# =======================================================
def load_DISAE(cwd):
    all_config = load_json(cwd + 'DTI_config.json')
...
# def evaluate(df, all_config, tokenizer, chem_dict, protein_dict, model,mode,loss_fn, datatype='dev'):
#     # collected_logits = []
#     # collected_labels = []
#     # for i in range(int(df.shape[0] / all_config['batch_size'])):
#         # print(i)
#     batch_logits, batch_labels,loss = core_batch_prediction_MAML(df,  all_config, tokenizer,mode,
#                                                             loss_fn,
#                                                             chem_dict,
#                                                        protein_dict, model,
#                                                        detach=True)
...
# def deal_Nonetype_parm(grad,model_param_shape):
#     #  deal with possible Nonetype param
#     n = 0
#     grad_list = []
#     for g in grad:
#         if type(g) == type(None):
#             print(n,' is Nonetype')
#             grad_list.append(torch.zeros(model_param_shape[n]))
#         else:
#             grad_list.append(g)
#         n = n + 1
#     return grad_list
...
#         n += 1
#         model_param_shape.append(param.shape)
#     return model_param_shape