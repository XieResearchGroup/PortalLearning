...
class interResidue_distMtx_classifier(nn.Module):
    def __init__(self,dim =20,
                 feat_mode='attentive-pool',pred_mode='binary',protein_descriptor=None,frozen='whole'):
        super(interResidue_distMtx_classifier, self).__init__()
        if pred_mode == 'binary':
            num_class = 2
        else:
            num_class = 9 # 9 for even prob 0.1 setting,34 for PDNET
        self.pred_DistMtx = distMtx_core_module(dim = dim,num_class=num_class,feat_mode=feat_mode)
        ...
        if frozen=='encoder-attention':
            print('frozen TAPE encoder attention layers')
            for n, m in self.protein_descriptor.named_children():
                if n == 'encoder':
                    for n_layer, m_layer in m.named_children():
                        for n_encoder, m_encoder in m_layer.named_children():
                            for n_core, m_core in m_encoder.named_children():
                                if n_core == 'attention':
                                    # print('yes')
                                    for param in m_core.parameters():
                                        param.requires_grad = False
    def forward(self,batch_input,device):
        if self.frozen=='whole':
            with torch.no_grad():
                embed_full = self.protein_descriptor(batch_input['tokenized-padded'])[0][:,1:-1,:]# with beining and end tokens
        else:
            # print('updating..')
            embed_full = self.protein_descriptor(batch_input['tokenized-padded'])[0][:, 1:-1,:]  # with beining and end tokens
        embed_binding_site = torch.bmm(batch_input['binding site selection matrix'], embed_full)
        logits = self.pred_DistMtx(embed_binding_site,embed_binding_site)
        return  logits
...
class DTI_distMtx_classifier(nn.Module):
    def __init__(self,dim =20,
        ...
        if chem_pretrained =='YES':
            ...
            self.chem_decriptor.load_state_dict(torch.load(cwd+contextpred_file))
        else:
