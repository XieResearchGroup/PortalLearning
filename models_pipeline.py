...
class DTI_pipeline(nn.Module):
    ...
    def forward(self, batch_protein_tokenized,batch_chem_graphs, **kwargs):
        ...
        elif self.all_config['protein_descriptor']=='TAPE':
            # batch_protein_repr = tape_batch_embedding(batch_protein_tokenized,self.proteinEmbedding,
            #                                           tape_related['tokenizer'],
            #                                           tape_related['protein_dict'])
            if self.all_config['frozen'] == 'whole':
                with torch.no_grad():
                    # print('TAPE all frozen!')
                    batch_protein_repr = self.proteinEmbedding(batch_protein_tokenized)[0]
            else:
                # print('TAPE being updated!')
                batch_protein_repr = self.proteinEmbedding(batch_protein_tokenized)[0]
            batch_protein_repr_compressed = self.prot_embed_transform(batch_protein_repr)
            batch_protein_repr_transformed = mask_tape_before_position_wise_attention(batch_protein_repr_compressed,
                                                                                           batch_protein_tokenized,
                                                                                           self.contextpred_config['emb_dim'],
                                                                                           self.use_cuda)
